...
\title{MATH 368/621 Fall 2019 Homework \#2}

\author{Professor Adam Kapelner} %STUDENTS: write your name here

\iftoggle{professormode}{
\date{Due under the door of KY604 11:59PM Friday, September 20, 2019 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\easysubproblem{Prove $\expe{\X + \c} = \muvec + \c$ where $\c \in \reals^K$, a constant.}\spc{2}
...
Draw from replacement 37 times. What is the probability of getting 10 red, 17 green, 6 blue and 4 yellow? Compute explicitly to the nearest two decimal places.}\spc{7}
...
\beqn
p_{X_j}(x_j) = \sum_{\x_{-j} \in \support{\X_{-j}}} p_{\X}(\x) = \binomial{n}{p_j}
\eeqn
...
\easysubproblem{Explain in English why $\B \sim \multinomial{1}{\p}$ is the multidimensional generalization of the Bernoulli r.v.}\\
\inred{This problem is duplicate. Skip.}
\spc{2}
...
\easysubproblem{If $\X \sim \multinomial{n}{\p}$, find $p_{\X_{-j}|X_{j}}(\x_{-j}, x_j)$ . This is marked easy since it's in the notes.}\spc{8}

\extracreditsubproblem{[MA] If $\X \sim \multinomial{n}{\p}$, find a proof for $\cov{X_i}{X_j} = -np_i p_j$ that is qualitatively different than the one we did in class.}\spc{9}
...
\beqn
\rho_{1,2} := \corr{X_1,\,X_2} := \frac{\sigma_{1,2}}{\sigma_1 \sigma_2} = \frac{\cov{X_1}{X_2}}{\se{X_1} \se{X_2}} = \frac{\cov{X_1}{X_2}}{\sqrt{\var{X_1} \var{X_2}}}
\eeqn

where $\se{\cdot}$ denotes the standard error of a r.v., the square root of its variance. Find $\corr{X_i,\,X_j}$ for two arbitrary elements in the r.v. vector $\X \sim \multinomial{n}{\p}$. }\spc{3}
