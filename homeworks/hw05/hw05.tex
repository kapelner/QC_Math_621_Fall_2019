...
\hardsubproblem{If somone has 5 kids, what is the probability they are Mormon according to our model?}\spc{6}
...
\hardsubproblem{Show that if $Y~|X=x \sim \exponential{x}$ and $X \sim \text{Gamma}(\alpha, \beta)$ then $Y \sim \text{Lomax}(\beta, \alpha)$. You will need to look up the Lomax distribution on wikipedia. Draw a tree of this model too.}\spc{10}

\hardsubproblem{[MA] Find the PDF of the compound distribution $Y$ if $Y~|~X_1=x_1,~X_2=x_2 \sim \betanot{x_1}{x_2}$ where $X_1 \sim \gammadist{\alpha_1}{\beta_1}$ independent of $X_2 \sim \gammadist{\alpha_2}{\beta_2}$. Draw a tree of this model. Get as far as you can.}\spc{6}
...
\intermediatesubproblem{Prove that all PDF's are $\in L^1$.}\spc{3}
...
Hint: use the Fourier inversion theorem and make a $u$ substitution.}\spc{3}
