...
\easysubproblem{Use (b) and (c) to show that given $\Xoneton \iid$ with finite $\mu$ but \emph{not finite} $\sigsq$, show that $\Xbar_n \convp \mu$, the \qu{strong} weak law of large numbers. This is marked easy because you can do this problem assuming you proved (b) and (c) but not actually having an answer for (b) and (c).}\spc{6}
...
\begin{enumerate}
\intermediatesubproblem{Let $Y | X = x \sim \normnot{a + bx}{\sigsq}$ where $a$ and $b$ are constants and $X \sim \exp{\lambda}$. Draw a picture of the joint density and the marginal densities along the axes.}\spc{7}

\intermediatesubproblem{Find $\expe{Y}$ via the law of iterated expectation.}\spc{5}
